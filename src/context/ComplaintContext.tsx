...
export const ComplaintProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    } else if (user?.id && role === 'technician') {
  ...
  const addComplaint = async (complaintData: any, attachments: FileList | null) => {
    try {
      ...
      const newComplaintData = {
        ...
        statusHistory: [{
          status: 'Open' as ComplaintStatus, timestamp: new Date().toISOString(), updatedBy: user?.id,
  ...
  const approveEstimate = async (complaintId: string) => {
    ...
    const updateDate: Pick<Complaint, "status" | "actualItems"> & {
      statusHistory: FieldValue;
    } = {
      status: 'Estimate Approved',
      statusHistory: arrayUnion(statusUpdate),
      actualItems: {
    ...
    if (complaint && complaint.actualItems.parts.length === 0 && complaint.estimatedItems.parts.length > 0) {
      updateDate.actualItems.parts = complaint.estimatedItems.parts;
    }
    if (complaint && complaint.actualItems.services.length === 0 && complaint.estimatedItems.services.length > 0) {
      updateDate.actualItems.services = complaint.estimatedItems.services;
    }
    await updateDoc(complaintRef, updateDate);
  }
